let cnv = document.getElementById("canvas"); //холст для рисованиея 
...
//устанавлеваем размер холста по размерам видимой части элемента canvas 
...
//сохранения холстов в виде картинки
document.getElementById("save_image").onclick = function(){
    ...
    //переносим на холст сохранения заднего фона
    ctx_save.drawImage(cnv_bg, 0, 0, cnv.width, cnv.height);
    //переносим на холст сохранения рисования
...
cnv.ontouchmove = (event) =>{
    if(tool == "Кисточка"){
        ctx.fillStyle = color;
        ctx.fillRect(event.changedTouches[0].pageX - width/2, event.changedTouches[0].pageY event   .offsetY- width/2, width, width);
    }
    else
        ctx.clearRect(event.changedTouches[0].pageX - width/2, event.changedTouches[0].pageY event.offsetY- width/2, width, width);      
}
//ждём события изменения цвета для политры заднего фона
background.addEventListener('input', changeBackground);
function changeBackground(){
    ctx_bg.fillStyle = background.value;
    ctx_bg.fillRect(0, 0, cnv_bg.width, cnv_bg.height);
}
//ждём события изменение инструмента 
